...
class _SellerDashboardPageState extends State<SellerDashboardPage> {
  ...
  void _listenStats() {
    ...
    _subOrdersSub = _firestore
      ...
      int    totalOrders = 0, pendingOrders = 0, shippingOrders = 0;
      int    deliveredOrders = 0, cancelledOrders = 0, totalQtySold = 0;
      ...
      for (final doc in snap.docs) {
        ...
        if (status == 'delivered') {
          ...
          totalQtySold += qty;
          final pid = (d['productId'] as String?) ?? (d['name'] as String? ?? doc.id);
          productAgg[pid] ??= {
            'productId': pid, 'name': d['name'] as String? ?? '—',
            'images': d['images'] as List<dynamic>? ?? [],
            'qtySold': 0, 'revenue': 0.0, 'orderCount': 0,
          };
          productAgg[pid]!['qtySold']    = (productAgg[pid]!['qtySold']    as int)    + qty;
          productAgg[pid]!['revenue']    = (productAgg[pid]!['revenue']    as double) + price * qty;
          productAgg[pid]!['orderCount'] = (productAgg[pid]!['orderCount'] as int)    + 1;
        }
        if (recent.length < 5) recent.add({...d, '_docId': doc.id});
      }

      final topList = productAgg.values.toList()
        ..sort((a, b) => (b['qtySold'] as int).compareTo(a['qtySold'] as int));
      ...
      setState(() {
        ...
        _avgOrderValue   = deliveredOrders > 0 ? totalRevenue / deliveredOrders : 0;
  ...
  Widget build(BuildContext context) {
    ...
    return Directionality( // ✅ wrapper racine
  ...
  Widget _buildAppBar(String Function(String) t, bool isRtl) {
    return SliverAppBar(
      expandedHeight: 120,
      floating: false,
      pinned: true,
      ...
      flexibleSpace: FlexibleSpaceBar(
        ...
        centerTitle: false,
        // ✅ EdgeInsetsDirectional : start = gauche en LTR, droite en RTL
        ...
        Padding(
          ...
          child: GestureDetector(
            ...
            child: Stack(clipBehavior: Clip.none, children: [
              ...
              if (_unreadCount > 0)
              // ✅ PositionedDirectional : end/start s'inversent en RTL
    ...
    return Column(
      ...
      children: [
        ...
        GridView.count(
          ...
          children: [
            _statCard(icon: Icons.inventory_2_rounded, label: t('seller_stat_products'),
                value: '$_totalProducts', color: const Color(0xFF3B82F6),
                bgColor: const Color(0xFFEFF6FF)),
            _statCard(icon: Icons.receipt_long_rounded, label: t('seller_stat_orders'),
                value: '$_totalOrders', color: const Color(0xFF8B5CF6),
                bgColor: const Color(0xFFF5F3FF)),
            _statCard(icon: Icons.attach_money_rounded, label: t('seller_stat_revenue'),
                value: currency.formatPrice(_totalRevenue),
                color: const Color(0xFF16A34A), bgColor: const Color(0xFFF0FDF4)),
            _statCard(icon: Icons.pending_actions_rounded, label: t('seller_stat_pending'),
                value: '$_pendingOrders', color: const Color(0xFFF59E0B),
                bgColor: const Color(0xFFFFFBEB), showBadge: _pendingOrders > 0),
          ],
        ),
      ],
    );
  ...
  Widget _statCard({required IconData icon, required String label,
    required String value, required Color color, required Color bgColor,
  ...
  Widget _buildOrderStatusBreakdown(String Function(String) t) {
    ...
    return Container(
      ...
      child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
        Row(children: [
          const Icon(Icons.pie_chart_rounded, color: Color(0xFF16A34A), size: 18),
        ...
        _statusBar(label: t('seller_status_paid'),    count: _pendingOrders,
            total: total, color: const Color(0xFFF59E0B), icon: Icons.pending_rounded),
        const SizedBox(height: 12),
        _statusBar(label: t('seller_status_shipping'), count: _shippingOrders,
            total: total, color: const Color(0xFF3B82F6),
            icon: Icons.local_shipping_rounded),
        const SizedBox(height: 12),
        _statusBar(label: t('seller_status_delivered'), count: _deliveredOrders,
            total: total, color: const Color(0xFF16A34A),
            icon: Icons.check_circle_rounded),
        const SizedBox(height: 12),
        _statusBar(label: t('seller_status_cancelled'), count: _cancelledOrders,
            total: total, color: const Color(0xFFDC2626), icon: Icons.cancel_rounded),
      ]),
    );
  }

  Widget _statusBar({required String label, required int count,
    ...
    return Row(children: [
      ...
      SizedBox(width: 28,
          child: Text('$count',
              textAlign: TextAlign.end, // ✅ end au lieu de right
  ...
  Widget _buildExtraStats(String Function(String) t, CurrencyProvider currency) {
    ...
    return Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
      ...
      const SizedBox(height: 16),
      Row(children: [
        Expanded(child: _miniStatCard(icon: Icons.local_mall_rounded,
            label: t('seller_stat_qty_sold'), value: '$_totalQtySold',
            color: const Color(0xFF0EA5E9), bgColor: const Color(0xFFE0F2FE))),
        const SizedBox(width: 12),
        Expanded(child: _miniStatCard(icon: Icons.trending_up_rounded,
            label: t('seller_stat_avg_order'),
            value: _avgOrderValue > 0 ? currency.formatPrice(_avgOrderValue) : '—',
            color: const Color(0xFF8B5CF6), bgColor: const Color(0xFFF5F3FF))),
        const SizedBox(width: 12),
        Expanded(child: _miniStatCard(icon: Icons.verified_rounded,
  ...
  Widget _miniStatCard({required IconData icon, required String label,
    required String value, required Color color, required Color bgColor}) {
    return Container(
      ...
      if (_topProducts.isEmpty)
        Container(padding: const EdgeInsets.all(28),
            ...
            child: Center(child: Column(children: [
              Icon(Icons.emoji_events_outlined, size: 44, color: Colors.grey[300]),
              const SizedBox(height: 10),
        ...
        ...(_topProducts.asMap().entries.map(
                (e) => _topProductTile(e.key, e.value, t, currency))),
    ]);
  }

  Widget _topProductTile(int rank, Map<String, dynamic> data,
      String Function(String) t, CurrencyProvider currency) {
    final medals     = ['🥇', '🥈', '🥉'];
    final rankColors = [const Color(0xFFFFB800), const Color(0xFF94A3B8),
      const Color(0xFFCD7F32)];
    final medal      = rank < medals.length ? medals[rank] : '${rank + 1}';
    final rankColor  = rank < rankColors.length ? rankColors[rank] : const Color(0xFF94A3B8);
    ...
    if (images.isNotEmpty) {
      try {
        ...
        imgWidget = ClipRRect(borderRadius: BorderRadius.circular(12),
            child: Image.memory(bytes, width: 60, height: 60, fit: BoxFit.cover));
      } catch (_) { imgWidget = _topImgPlaceholder(rankColor); }
    } else { imgWidget = _topImgPlaceholder(rankColor); }

    final maxQty   = _topProducts.isNotEmpty ? _topProducts.first['qtySold'] as int : 1;
    ...
    return Container(
      ...
      decoration: BoxDecoration(color: Colors.white,
          borderRadius: BorderRadius.circular(16),
          border: rank == 0 ? Border.all(
              color: const Color(0xFFFFB800).withOpacity(0.4), width: 1.5) : null,
          boxShadow: [BoxShadow(
              color: rank == 0 ? const Color(0xFFFFB800).withOpacity(0.10)
      ...
      child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
        Row(crossAxisAlignment: CrossAxisAlignment.start, children: [
          ...
          Stack(children: [
            imgWidget,
            if (rank == 0)
            // ✅ PositionedDirectional — badge étoile RTL-aware
              PositionedDirectional(top: -4, end: -4,
                child: Container(width: 18, height: 18,
                    decoration: BoxDecoration(color: const Color(0xFFFFB800),
          ...
          Expanded(child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
            ...
            Wrap(spacing: 8, runSpacing: 4, children: [
              _topChip(icon: Icons.shopping_bag_rounded,
                  label: '$qtySold ${t("seller_units_sold")}', color: rankColor),
              _topChip(icon: Icons.receipt_rounded,
  ...
  Widget _topChip({required IconData icon, required String label, required Color color}) {
    return Container(
  ...
  Widget _topImgPlaceholder(Color color) => Container(
    ...
    child: Icon(Icons.inventory_2_rounded, color: color.withOpacity(0.5), size: 26),
  );
    ...
    return Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
      Row(mainAxisAlignment: MainAxisAlignment.spaceBetween, children: [
        ...
        Row(children: [
          ...
          Text('Live', style: TextStyle(fontSize: 11, color: Colors.grey[500],
      ...
      if (_recentSubOrders.isEmpty)
        ...
        ...(_recentSubOrders.map((d) => _subOrderTile(d, t, currency))),
    ]);
  }

  Widget _subOrderTile(Map<String, dynamic> data,
      String Function(String) t, CurrencyProvider currency) {
    ...
    if (images.isNotEmpty) {
      ...
      } catch (_) { leading = _tilePlaceholder(status); }
    } else { leading = _tilePlaceholder(status); }
    ...
    return Container(
      ...
      child: Row(children: [
        ...
        Expanded(child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
          ...
          const SizedBox(height: 2),
          Row(children: [
            Text('#$shortId', style: TextStyle(fontSize: 11, color: Colors.grey[400])),
          ...
          Container(padding: const EdgeInsets.symmetric(horizontal: 7, vertical: 2),
              decoration: BoxDecoration(color: statusColor.withOpacity(0.10),
                  borderRadius: BorderRadius.circular(6)),
              child: Text(t('seller_status_$status'),
  ...
  Widget _tilePlaceholder(String status) {
    final cfg = _statusConfig(status);
    return Container(width: 46, height: 46,
        decoration: BoxDecoration(color: (cfg['color'] as Color).withOpacity(0.10),
            borderRadius: BorderRadius.circular(10)),
  ...
  Map<String, dynamic> _statusConfig(String status) {
    switch (status) {
      case 'paid':      return {'color': const Color(0xFFF59E0B), 'icon': Icons.pending_rounded};
      case 'shipping':  return {'color': const Color(0xFF3B82F6), 'icon': Icons.local_shipping_rounded};
      case 'delivered': return {'color': const Color(0xFF16A34A), 'icon': Icons.check_circle_rounded};
      case 'cancelled': return {'color': const Color(0xFFDC2626), 'icon': Icons.cancel_rounded};
      default:          return {'color': const Color(0xFF94A3B8), 'icon': Icons.receipt_rounded};
    }
  }
}